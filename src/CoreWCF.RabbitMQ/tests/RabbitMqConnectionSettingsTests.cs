...
public class RabbitMqConnectionSettingsTests : IClassFixture<RabbitMqConnectionSettingsFixture>
{
    ...
    [Fact]
    public void ConnectionSettingsFromStandardUri_Populates_UserName()
    {
        Assert.Equal("username", _fixture.ConnectionSettingsFromStandardUri.UserName);
    }

    [Fact]
    public void ConnectionSettingsFromStandardUri_Populates_Password()
    {
        Assert.Equal("password", _fixture.ConnectionSettingsFromStandardUri.Password);
    }
    ...
    {
        Assert.StartsWith(RabbitMqConnectionSettings.TempQueuePrefix, _fixture.ConnectionSettingsFromUriWithNoQueueName.QueueName);

        var queueNameSuffix = _fixture.ConnectionSettingsFromUriWithNoQueueName.QueueName
            .Replace(RabbitMqConnectionSettings.TempQueuePrefix, string.Empty);
