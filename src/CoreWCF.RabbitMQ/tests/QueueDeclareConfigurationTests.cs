...
public class QueueDeclareConfigurationTests : IClassFixture<QueueDeclareConfigurationFixture>
{
    ...
    [Fact] public void ClassicQueueConfiguration_Does_Not_Throw_If_QueueType_Is_Unchanged()
    {
        var exception = Record.Exception(() =>
            _fixture.ClassicQueueConfiguration.QueueType = RabbitMqQueueType.Classic);
        Assert.Null(exception);
    }

    [Fact] public void ClassicQueueConfiguration_Throws_If_QueueType_Is_Changed()
    {
        Assert.Throws<ArgumentException>(() =>
            _fixture.ClassicQueueConfiguration.QueueType = "");
    }
    ...
    [Fact] public void QuorumQueueConfiguration_Does_Not_Throw_If_QueueType_Is_Unchanged()
    {
        var exception = Record.Exception(() =>
            _fixture.QuorumQueueConfiguration.QueueType = RabbitMqQueueType.Quorum);
        Assert.Null(exception);
    }

    [Fact] public void QuorumQueueConfiguration_Throws_If_QueueType_Is_Changed()
    {
        Assert.Throws<ArgumentException>(() =>
            _fixture.QuorumQueueConfiguration.QueueType = "");
    }
