...
namespace CoreWCF.Channels
{
    public class RabbitMqTransportPump : QueueTransportPump
    {
        ...
        {
            var context = new RabbitMqQueueMessageContext
            {
                ...
                DispatchResultHandler = DispatchResultCallbackAsync,
                DeliveryTag = deliveryTag
            };
            return context;
        }

        private async Task DispatchResultCallbackAsync(QueueDispatchResult dispatchResult, QueueMessageContext queueMessageContext)
        {
            var context = queueMessageContext as RabbitMqQueueMessageContext;
            Fx.Assert(context != null,
                $"Expected a {nameof(RabbitMqQueueMessageContext)} but received an instance of {queueMessageContext.GetType().Name} instead.");

            if (dispatchResult == QueueDispatchResult.Processed && !_isAutoAck)
            {
                await Task.Run(() => _channel.BasicAck(context.DeliveryTag, false));
            }

            if (dispatchResult == QueueDispatchResult.Failed)
            {
                _logger.LogError($"Dispatch failed for message with delivery tag: {context.DeliveryTag}");
            }
        }
    }
}
